...
public abstract class CpuSpecificationParser {
    ...
    public static CpuSpecificationModel extractSpecification(String url) throws IOException, ElementNotFoundException {
        ...
        Element multithreadLabelElement = page.selectFirst("div:containsOwn(Multithread Rating)");
        if (multithreadLabelElement != null) {
            Element multithreadValueElement = multithreadLabelElement.nextElementSibling();
            if (multithreadValueElement != null) {
                specification.multithreadRating = multithreadValueElement.text().trim();
            }
        }

        Element singleThreadLabelElement = page.selectFirst("div:containsOwn(Single Thread Rating)");
        if (singleThreadLabelElement != null) {
            Element singleThreadValueElement = singleThreadLabelElement.nextElementSibling();
            if (singleThreadValueElement != null) {
                specification.singleThreadRating = singleThreadValueElement.text().trim();
            }
        }
